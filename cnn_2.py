...
def main(_):
    ...
    with tf.name_scope('input'):
        ...
        y_expect = tf.reshape(y_, [-1, NUM_PER_IMAGE, LABEL_SIZE])
    ...
    with tf.name_scope('readout'):
        ...
        y_conv_flat = tf.matmul(h_fc1_drop, W_fc2) + b_fc2

        y_conv = tf.reshape(y_conv_flat, [-1, NUM_PER_IMAGE, LABEL_SIZE])
    ...
    with tf.name_scope('loss'):
        cross_entropy = tf.reduce_mean(
            ...
            tf.nn.softmax_cross_entropy_with_logits(labels=y_expect, logits=y_conv))
        train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
        variable_summaries(cross_entropy)

    # forword prop
    predict = tf.argmax(y_conv, axis=2)
    expect = tf.argmax(y_expect, axis=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='images/char-2-groups-200/',
